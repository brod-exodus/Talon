...
export async function PATCH(
  ...
  try {
    ...
    console.error("[ecosystems/[id]/lists/[listId]] PATCH error:", error)
    return NextResponse.json({ error: "Failed to rename project list" }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("Default team is missing")) return teamContextError(error)
    if (error instanceof Error && error.message.includes("not a member")) return teamContextError(error)
    console.error("[ecosystems/[id]/lists/[listId]] DELETE error:", error)
    return NextResponse.json({ error: "Failed to delete project list" }, { status: 500 })
  }
}
