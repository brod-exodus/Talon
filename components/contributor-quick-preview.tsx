...
export function ContributorQuickPreview({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="left-auto right-0 top-0 h-screen max-w-xl translate-x-0 translate-y-0 overflow-y-auto rounded-none border-border bg-popover p-0 shadow-none  sm:rounded-none">
        <div className="p-3">
          ...
          <div className="mt-6 space-y-4">
            <section className="rounded-lg border border-border bg-card p-3 shadow-none">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="truncate text-xl font-semibold text-foreground">{display.name}</h3>
                  ...
                  {!currentProject && (
                    <div className="space-y-2">
                      ...
                      {projectChoices.length > 0 ? (
                        ...
                        <p className="rounded-lg border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm font-medium text-amber-300">
                  ...
                  {selectedProjectId && (
                    <>
                      <div className="space-y-2">
                        <Label>List</Label>
                        {listsLoading ? (
                          <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/10 px-4 py-3 text-sm font-semibold text-primary">
            ...
            <section className="grid gap-3 sm:grid-cols-3">
              {stats.length > 0 ? (
                stats.map((stat) => (
                  <div key={stat.label} className="rounded-lg border border-border bg-card p-3 shadow-none">
                    <p className="text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">{stat.label}</p>
                    <p className="mt-2 text-lg font-semibold text-foreground">{stat.value}</p>
                  </div>
                ))
              ) : (
                <div className="rounded-lg border border-border bg-card p-3 text-sm font-semibold text-muted-foreground sm:col-span-3">
            ...
            {error && (
              <div className={cn("rounded-lg border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm font-medium text-amber-300")}>
                {error}
              </div>
            )}
          </div>
